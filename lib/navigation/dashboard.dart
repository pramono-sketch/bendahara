...
class _DashboardPageState extends ConsumerState<DashboardPage> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      _hasError = true;
      if (mounted) {
        showTopNotification(context, 'Tidak ada koneksi internet. Coba lagi.');
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  Widget _buildSectionHeader(String title, AppThemeMode themeMode) {
  ...
  Widget _buildSectionGroup(AppThemeMode themeMode, List<Widget> children) {
    if (_isNeo(themeMode)) {
      return Container(
  ...
  Widget _buildStudentPaymentCard(AppThemeMode themeMode) {
    final theme = Theme.of(context);
    final colors = theme.colorScheme;
    final paidStudents = _activeStudents.where((s) => !s.hasOutstanding).length;
    final totalStudents = _activeStudents.length;
    final persentaseLunas = totalStudents > 0 ? (paidStudents / totalStudents * 100) : 0;

    return _buildSectionGroup(themeMode, [
      Padding(
        padding: const EdgeInsets.all(16),
        child: Row(
          children: [
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Siswa Lunas', style: theme.textTheme.bodyMedium),
                  const SizedBox(height: 4),
                  Text(
                  ...
                  ClipRRect(
                    ...
                    child: LinearProgressIndicator(
                      value: totalStudents > 0 ? paidStudents / totalStudents : 0,
  ...
  List<String> _getLast6MonthsLabels() {
    List<String> monthLabels = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des'];
    final now = DateTime.now();
    List<String> labels = [];
    for (int i = 5; i >= 0; i--) {
  ...
  Widget _buildBarChart(List<String> labels, List<double> values, AppThemeMode themeMode) {
    final theme = Theme.of(context);
    final colors = theme.colorScheme;
      ...
      BarChartData(
        ...
        titlesData: FlTitlesData(
          ...
          leftTitles: AxisTitles(
            sideTitles: SideTitles(
              showTitles: true,
              reservedSize: 40, // Diperbesar agar tidak menumpuk dengan batang
              interval: maxY / 4,
              getTitlesWidget: (value, meta) {
